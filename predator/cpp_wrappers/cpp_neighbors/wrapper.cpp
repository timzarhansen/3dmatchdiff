#define NPY_NO_DEPRECATED_API NPY_1_7_API_VERSION  // Keep this first!
#include <Python.h>
...
//    static const char* kwlist[] = { "queries", "supports", "q_batches", "s_batches", "radius", NULL };
...
//	PyObject* queries_array = PyArray_FROM_OTF(queries_obj, NPY_FLOAT, NPY_IN_ARRAY);
    ...
    if ((int)PyArray_NDIM((PyArrayObject*)queries_array) != 2 ||
        (int)PyArray_DIM((PyArrayObject*)queries_array, 1) != 3)
	{
		Py_XDECREF(queries_array);
		Py_XDECREF(supports_array);
		Py_XDECREF(q_batches_array);
...
//	queries = vector<PointXYZ>((PointXYZ*)PyArray_DATA(queries_array), (PointXYZ*)PyArray_DATA(queries_array) + Nq);
	queries = vector<PointXYZ>(
    (PointXYZ*)PyArray_DATA((PyArrayObject*)queries_array),
    (PointXYZ*)PyArray_DATA((PyArrayObject*)queries_array) + Nq);
...
//	memcpy(PyArray_DATA(res_obj), neighbors_indices.data(), size_in_bytes);
    memcpy(
    PyArray_DATA((PyArrayObject*)res_obj),
