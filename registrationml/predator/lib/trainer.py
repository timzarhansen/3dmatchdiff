...
from registrationml.predator.lib.timer import Timer, AverageMeter
from registrationml.predator.lib.utils import Logger, validate_gradient
...
class Trainer(object):
    ...
    def inference_one_batch(self, inputs, phase):
        ...
        if (phase == 'train'):
            ...
            ###################################################
            # get loss
            stats = self.desc_loss(src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence, c_rot, c_trans,
                                   scores_overlap, scores_saliency)

            c_loss = stats['circle_loss'] * self.w_circle_loss + stats['overlap_loss'] * self.w_overlap_loss + stats[
