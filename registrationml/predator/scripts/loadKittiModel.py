...
cwd = os.getcwd()
sys.path.append(cwd)
from  registrationml.predator.datasets.indoor import IndoorDataset
from  registrationml.predator.datasets.dataloader import get_dataloader
from  registrationml.predator.models.architectures import KPFCNN
from  registrationml.predator.lib.utils import load_obj, setup_seed,natural_key, load_config
from  registrationml.predator.lib.benchmark_utils import ransac_pose_estimation, to_o3d_pcd, get_blue, get_yellow, to_tensor
from  registrationml.predator.lib.trainer import Trainer
from  registrationml.predator.lib.loss import MetricLoss
import shutil
setup_seed(0)
...
class ThreeDMatchDemo(Dataset):
    ...
    def __getitem__(self,item): 
        ...
        # R = pcd1.get_rotation_matrix_from_xyz((0 , 0, 0))
...
if __name__ == '__main__':
    ...
    pcd1 = o3d.io.read_point_cloud(config.src_pcd)
    pcd2 = o3d.io.read_point_cloud(config.tgt_pcd)

    pcd2 = copy.deepcopy(pcd1)
    pcd1.translate((-1.3, 1, -2))
    R = pcd1.get_rotation_matrix_from_xyz((-np.pi/2 , 0, 0))
    pcd1.rotate(R, center=(0, 0, 0))
    ...
    demo_set = ThreeDMatchDemo(config, pcd1, pcd2)
    ...
    # [561 , 54 , 77 , 90] [569 , 50 , 75 , 89] [540 , 59 , 76 , 79]
    ...
    # inputs1 = create_input_dict(config.src_pcd,config.tgt_pcd,config)
    # inputs3 = calculateOutput(config.src_pcd,config.tgt_pcd)
    # # inputs12 = dict(inputs1)
    # c_loader_iter = demo_loader.__iter__()
    #
    #
    # inputs2 = next(c_loader_iter)
    #
    ...
    #     if type(v) == list:
    #         inputs2[k] = [item.to(config.device) for item in v]
    #     else:
    #         inputs2[k] = v.to(config.device)
    #
    # for k, v in inputs1.items():
    #     if type(v) == list:
    #         inputs1[k] = [item.to(config.device) for item in v]
    #     else:
    #         inputs1[k] = v.to(config.device)
    # for k, v in inputs3.items():
    #     if type(v) == list:
    #         inputs3[k] = [item.to(config.device) for item in v]
    #     else:
    #         inputs3[k] = v.to(config.device)

    # feats2, scores_overlap2, scores_saliency2 = config.model(inputs2)  #[N1, C1], [N2, C2]
    # feats1, scores_overlap1, scores_saliency1 = config.model(inputs1)  #[N1, C1], [N2, C2]
    # feats3, scores_overlap3, scores_saliency3 = config.model(inputs3)  #[N1, C1], [N2, C2]
